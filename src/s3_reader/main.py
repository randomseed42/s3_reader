...
from .s3_reader import download_by_keys, download_by_prefixes, list_keys_by_prefixes, list_prefixes_by_prefixes
...
def main():
    ...
    parser_list.add_argument('-k', '--keys', action='store_true',
                             help='get keys if True else get prefixes, default get prefixes')
    ...
    if args.func == 's3list':
        if args.keys:
            ...
            prefixes = list_prefixes_by_prefixes(
                bucket=args.bucket,
                prefixes=args.prefixes,
            )
            print(prefixes)
